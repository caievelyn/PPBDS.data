...
Create a list-column of weights called `weight_groups`, grouping by `Diet` and `Time`. Save this list-column in a new dataframe called `x`.
...
Use a `map_*` function to convert all of the temperatures in `monthly_temp` from Fahrenheit to Celsius, creating a new variable `temp_celsius`. Hint: The formula to convert Fahrenheit to Celsius is `(. - 32) * 5/9`.
