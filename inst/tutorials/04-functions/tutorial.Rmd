...
Create a list-column of `weight`s using `mutate()`called `weight_groups`, grouping by `Diet` and `Time`. Save this list-column in a new dataframe called `x`.
...
## Argument Specifications and a Data Generating Mechanism
...
```{r mysteryfunc2-setup, exercise.setup = "mysteryfunc-setup"}

```

```{r mysteryfunc2, exercise = TRUE}
